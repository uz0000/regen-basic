"""
Estimand preservation — regression-coefficient recovery.

A caller declares an analysis (``EstimandSpec``: ``outcome ~ predictors``,
family ols|logit). Fit it on the **real** reference to get theta_real +/- CI,
fit the *same* spec on the **synthetic** data to get theta_synth, and certify
that each coefficient-of-interest's theta_synth is statistically
indistinguishable from theta_real. This is a guarantee distinct from fidelity
(marginals/correlations): a batch can pass every fidelity check while a
coefficient silently shifts.

Self-contained (numpy + scipy.stats) — no statsmodels, no sklearn solver — so
the fit is deterministic to a fixed tolerance on any machine. OLS is the
closed-form normal equations with a t-interval; logit is Newton/IRLS with a
Wald interval from the inverse Fisher information.
"""
...
def _design(df, spec: EstimandSpec) -> Tuple[np.ndarray, np.ndarray, List[str], int]:
    ...
    Requires numeric outcome + predictors; rows with any NaN in the used
    columns are dropped. Coefficient names are ["(Intercept)", *predictors].
    """
    ...
    if nonnum:
        raise EstimandError(
            f"estimand supports numeric columns only; non-numeric: {nonnum}")
    sub = sub.dropna()
    n = len(sub)
    p = len(spec.predictors) + 1  # + intercept
    if n <= p:
        raise EstimandError(f"too few complete rows ({n}) to fit {p} parameters")
...
def _fit_logit(X, y, names, ci_level) -> Dict[str, Any]:
    ...
    if not uniq.issubset({0.0, 1.0}):
        raise EstimandError(f"logit outcome must be binary 0/1; saw {sorted(uniq)[:5]}")
    n, p = X.shape
    beta = np.zeros(p)
...
def _pack(names, beta, se, crit, n, dof, family) -> Dict[str, Any]:
    ...
    for name, b, s in zip(names, beta, se):
        b, s = float(b), float(s)
        coefs[name] = {"coef": b, "se": s, "ci_low": b - crit * s, "ci_high": b + crit * s}
    return {"family": family, "n": int(n), "dof": int(dof), "coefficients": coefs}


def fit_estimand(df, spec: EstimandSpec) -> Dict[str, Any]:
    """Fit the declared estimand on ``df``.

    Deterministic given the data. Raises ``EstimandError`` if the spec cannot
    be fit (missing/non-numeric columns, too few rows, non-binary logit
    outcome).
    """
...
def certify(real_fit: Dict[str, Any], synth_fit: Dict[str, Any],
    ...
    """Decide, per coefficient of interest, whether theta_synth preserves theta_real.

    Rule ``consistent`` (default): preserved iff the two estimates are not
    distinguishable beyond their combined standard error — a two-sample Wald
    test, ``|theta_real - theta_synth| <= z * sqrt(se_real^2 + se_synth^2)`` at
    level ``ci_level``. This accounts for both samples' uncertainty, so it
    does not false-fail on genuinely preserved data.

    Rule ``within_ci`` (stricter): preserved iff theta_synth is inside
    theta_real's CI.
    """
    ...
    for name in spec.targets():
        ...
        # If theta_real's own CI includes 0, there is no real signal to
        # preserve — "preservation" of a null effect is vacuous. Surfaced,
        # not (yet) failed.
...
def reference_aggregate(real_fit: Dict[str, Any], spec: EstimandSpec) -> Dict[str, Any]:
    """The disclosable theta_real +/- SE aggregate — no per-row values — that
    lets a third party recompute the certificate from synthetic data alone."""
    return {
...
def evaluate(real_df, synth_df, spec: EstimandSpec):
    """Full estimand assessment. **Never raises** — a spec that cannot be fit
    becomes a status, not an exception (fail loud, not crash).

    Returns ``(assessment, real_fit)``. ``assessment['status']`` is one of
    ``not_declared`` | ``uncertifiable`` | ``certified`` | ``not_preserved``.
    """
    ...
    try:
        ...
        # No credible theta_real (or theta_synth) -> refuse to certify, never
        # fake it.
